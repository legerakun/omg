...
fText = {}
fText.Scroll = Material("hud/scrollbar/vert_marker.vtf")
fText.Up = Material("hud/scrollbar/arrow_up.vtf")
fText.Dwn = Material("hud/scrollbar/arrow_down.vtf")
...
function PANEL:Init()
			...
			if liney + 2*me.chartall > me.VBar:GetScroll() and liney + 2*me.chartall < me.VBar:GetScroll() + me:GetTall() + me.chartall then
				...
				for i_n=1, #l_v do
					...
					if i_v[1] == "text" then
						...
						local txt = i_v[2].text
						local font2 = font.."_Blur"
						surface.SetFont(font2)
						local fw,fh = surface.GetTextSize("|")
						--color = Schema.GameColor
						local color2 = Color(color.r*.5, color.g*.5, color.b*.5)
						
						self:PaintTextpart( txt, font2, lastx, liney + ctall, color2 )		
						self:PaintTextpart( txt, font, lastx, liney + ctall, color )			
  ...
  local bar = self.VBar
  bar.Paint = function(me)
	return
  end
  bar.btnGrip.Paint = function(me)
    local col = Schema.GameColor
    surface.SetDrawColor(col)
    surface.SetMaterial(fText.Scroll)
    surface.DrawTexturedRect(0,0,me:GetWide(),me:GetTall())
  end
  bar.btnUp.Paint = function(me)
    local col = Schema.GameColor
    surface.SetDrawColor(col)
    surface.SetMaterial(fText.Up)
    surface.DrawTexturedRect(0,0,me:GetWide(),me:GetTall()*.65)    
  end
  bar.btnDown.Paint = function(me)
    local col = Schema.GameColor
    surface.SetDrawColor(col)
    surface.SetMaterial(fText.Dwn)
    surface.DrawTexturedRect(0,0,me:GetWide(),me:GetTall()*.65)      
  end
